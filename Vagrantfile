...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    yum update -y
    yum install -y iftop iotop bind bind-utils net-tools mc
	echo "127.0.0.1 localhost" | sudo tee /etc/hosts
	echo "127.0.0.1 localhost localhost.localdomain localhost4 localhost4.localdomain4" | sudo tee /etc/hosts
	echo "::1 " localhost localhost.localdomain localhost6 localhost6.localdomain6" | sudo tee /etc/hosts
	echo "192.168.56.10 srv1" | sudo tee /etc/hosts
    echo "192.168.56.11 srv2" | sudo tee /etc/hosts
  SHELL
